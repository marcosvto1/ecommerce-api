module Storefront::V1
  class WishItemsController < ApiController
    def index
      @wish_items = current_user.wish_items.joins(:product).includes(:product)
    ...
    def wish_item_params
      wish_params = params.require(:wish_item).permit(:product_id)
      wish_params.merge!({user_id: current_user.id })
      wish_params
    end
  end
end