...
RSpec.describe Order, type: :model do
  ...
  it { is_expected.to define_enum_for(:payment_type).with_values({
                                                                   credit_card: 1, billet: 2
                                                                 })}
  ...
  it "should #due_date must be 7 days ahead :created_at" do
    subject = create(:order)
    day_seconds = 86400
    distance_days = (subject.due_date - subject.created_at) / day_seconds
    expect(distance_days).to eq 7
  end

  it "should receives #pending_payment status as default on creation" do
